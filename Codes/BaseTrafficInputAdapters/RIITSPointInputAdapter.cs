...
namespace BaseTrafficInputAdapters
{
    public abstract class RIITSPointInputAdapter : PointInputAdapter
    {
        ...
        protected long _eventsEnqueued;
        ...
        protected PointEvent ConvertRecordToEvent(List<string> fields)
        {
            ...
            try
            {
                ...
                for (int ordinal = 0; ordinal < _bindtimeEventType.FieldsByOrdinal.Count; ordinal++)
                {
                    ...
                    {
                        if (evtField.Type.ClrType == typeof(string) && fields[ordinal] == "")
                        {
                            value = "";
                            evt.SetField(cepOrdinal, "");
                        }
                        ...
                        //ExceptionEmailReporter reporter = new ExceptionEmailReporter(e.Message, 117, "AzureTableStorageMediaWriter.cs");
        ...
        public override void Resume()
        {
            if (parser != null)
            {
                StreamWriter tempWriter = new StreamWriter(parser.DataType + "RIITSPointInputAdapter.txt", true);
                tempWriter.WriteLine("Resume Time: " + DateTime.Now.ToString());
                tempWriter.Close();
                tempWriter.Dispose();
            }
        ...
        protected bool jjEnqueueCTI(DateTimeOffset value)
        {
            ...
            if (EnqueueOperationResult.Full == result)
            {
                ...
                StreamWriter tempWriter = new StreamWriter(parser.DataType + "RIITS_CTI_Enqueue_resume.txt", true);
                tempWriter.WriteLine("Resume Time: " + DateTime.Now.ToString());
                tempWriter.Close();
                tempWriter.Dispose();
